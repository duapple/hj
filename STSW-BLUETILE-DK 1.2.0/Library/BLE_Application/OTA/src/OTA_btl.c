...
tBleStatus OTA_Add_Btl_Service(void) {
	...
	ret = aci_gatt_add_char(btlServHandle, UUID_TYPE_128, &ota_char_uuid, 8, CHAR_PROP_READ, ATTR_PERMISSION_NONE, GATT_NOTIFY_READ_REQ_AND_WAIT_FOR_APPL_RESP, 16, 0, &btlImageCharHandle);
	if (ret != BLE_STATUS_SUCCESS)
		goto fail;
	...
	ret = aci_gatt_add_char(btlServHandle, UUID_TYPE_128, &ota_char_uuid, 9, CHAR_PROP_READ | CHAR_PROP_WRITE | CHAR_PROP_WRITE_WITHOUT_RESP, ATTR_PERMISSION_NONE, GATT_NOTIFY_READ_REQ_AND_WAIT_FOR_APPL_RESP | GATT_NOTIFY_ATTRIBUTE_WRITE, 16, 0, &btlNewImageCharHandle);
	if (ret != BLE_STATUS_SUCCESS)
		goto fail;
	...
	ret = aci_gatt_add_char(btlServHandle, UUID_TYPE_128, &ota_char_uuid, IMAGE_CONTENT_SIZE, CHAR_PROP_READ | CHAR_PROP_WRITE | CHAR_PROP_WRITE_WITHOUT_RESP, ATTR_PERMISSION_NONE, GATT_NOTIFY_READ_REQ_AND_WAIT_FOR_APPL_RESP | GATT_NOTIFY_ATTRIBUTE_WRITE, 16, 1, &btlNewImageTUContentCharHandle);
	if (ret != BLE_STATUS_SUCCESS)
		goto fail;
	...
	ret = aci_gatt_add_char(btlServHandle, UUID_TYPE_128, &ota_char_uuid, 4, CHAR_PROP_NOTIFY | CHAR_PROP_READ, ATTR_PERMISSION_NONE, GATT_NOTIFY_READ_REQ_AND_WAIT_FOR_APPL_RESP, 16, 0, &btlExpectedImageTUSeqNumberCharHandle);
	if (ret != BLE_STATUS_SUCCESS)
		goto fail;
 ...
 * @note The API code could be subject to change in future releases.
 */
void OTA_Write_Request_CB(uint16_t connection_handle, uint16_t attr_handle, uint8_t data_length, uint8_t *att_data) {
	tBleStatus ret;
	uint16_t k;
